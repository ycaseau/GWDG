...
[getNeed(c:Consumer,y:Year) : void
  -> let b := c.economy,
      ...
      (//[SHOW2] === needs(~S) = ~F2 PWh vs ~F2 (gdp:~F%) // c,c2,c0,globalEconomyRatio(b,y),
...
// also new in CCEM v8: add a 
[roI(b:Block,y:Year) : Percent
  -> pb.world.returnOnInvestment * get(b.roiEfficiency,yearF(y)) *
     getCompetitiveness(b,y) *
     ((1 - get(b.socialExpenseRatio,yearF(y))) ^ 2.0)]

// debug : showROI
[showRoi(b:Block,y:Year)
   -> printf("=== ROI for block ~S in year ~A is ~F2%=~F% * ~F2% * ~F2% * [~F2%:~F2%]\n",b,year!(y),
             roI(b,y),pb.world.returnOnInvestment, get(b.roiEfficiency,yearF(y)),
             (1 - get(b.socialExpenseRatio,yearF(y))) ^ 2.0,
             getCompetitiveness(b,y), laborFactor(b,y))]

// competitiveness is a KNU (affine) that takes labor cost (proxy : gdp/h) as input
[laborFactor(b:Block,y:Year) : Percent
...
[getCompetitiveness(b:Block,y:Year) : Percent
   -> get(pb.world.competitivenessFactor, laborFactor(b,y))]
...
// in CCEM v8, we ignore populationGrowth but keep the productivityLoss effect
...
[computeInvest(b:Block,y:Year) : void
   -> let  iv := b.investGrowth[y - 1],      // last year invest
         ...
         (//[SHOW4] M4: ~S invest ~F% of GDP (grows from ~F2T$ to ~F2T$ = ~F%), target=~F% // b.describes, (iv / r1), r1, r2, (r2 - r1) / r2, b.iRevenue,
          ...
          b.investGrowth[y] := (ix - invAdapt + b.describes.carbonTaxes[y]),  // CCEM v8 : was (ix - invE - invAdapt)
...
[showProduction(s:Supplier,y:Year,cMax:Energy) : void
  -> printf("[~A] prodShow(~S) cMax=~F2(adds:~F2); needs=~F2TWh\n",year!(y),s,
            s.capacities[y],s.additions[y - 1],sum(list{c.needs[y][s.index] | c in Consumer}))] 
...
[showMaxCapacity(x:FiniteSupplier,y:Year,cMax:Energy) : void
  -> let prev := prevMaxCapacity(x,y),
         I1 := x.inventories[y - 1],                   // inventory with that price, minus gone
         rProd := prodGrowth(x,prev,y),          // what growth should be
   ...
   (printf("[~A] >>> inventory(~S) = ~F2; ~F2 (gone)\n",year!(y),x,I1,x.gone),
    printf("[~A] >>> max capacity(~S)=~F2 vs ~F2 (formula) ->(prev:~F2, inventory ratio: ~F% & rProd = ~F% => rGrowth=~F%) Gtep {was:~F2}\n",
            year!(y),x,cMax,c,prev,(I1 / x.threshold), rProd, rGrowth,x.capacities[y - 1])) ]

// same for InfiniteSupplier
[showMaxCapacity(x:InfiniteSupplier,y:Year,cMax:Energy) : void
  -> let  prev := prevMaxCapacity(x,y) - x.additions[y - 1],   // we want to apply maxDelta to all
         ...
         printf("[~A] >>> max capacity(~S)=~F2  (rProd=~F%,maxD=~F2 => growth=~F2) PWh {was:~F2}\n",
            year!(y),x,prev + growth, rProd, maxDelta, growth, prev) ]
