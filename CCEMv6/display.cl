...
[lookSolve(s:Supplier)
  ->  let y := pb.year in
     (pb.prodCurve := list<Price>{ getOutput(s,pb.priceRange[i],y) | i in (1 .. NIS)},
      ...
      let p1 :=  TESTsolve(pb,s) in          // M2: find the approximate equilibrium price
...
// this is summer 2024 code, to be replaced with 8 KNUS analysis (kpi)
[knus()
...
[elasticity() : float
  -> let s := 0.0, gsum := 0.0 in
       (for c in Consumer
          (s :+  c.economy.gdp * elasticity(c),
           gsum :+ c.economy.gdp),
        s / gsum) ]  // average kn3 weighted by gdp

[elasticity(c:Consumer) : float
...
// global saving ratio
[savingRatio(y:Year) : float
  -> let s := 0.0, gsum := 0.0 in
       (for c in Consumer
          (s :+  c.economy.gdp * savingRatio(c,y),
           gsum :+ c.economy.gdp),
        s / gsum) ]  
...
defau :: Experiment(
  comment = "default",
  init = (n){print("-- default parameters --\n")}
)
