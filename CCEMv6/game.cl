...
DI:boolean :: true      // use disolve vs solve for the market equilibrium
...
[getOutput5(s:FiniteSupplier,p:Price,cMax:Energy,y:Year) : float
...
[getOutput(s:Supplier,p:Price,y:Year) : float
   -> getOutput(s,p,getMaxCapacity(s,y),y)]

// to void oscillation, we average the previous max capacity and the new one
[getMaxCapacity(s:FiniteSupplier,y:Year) : Energy
   ->  0.5 * (capacity(s,y,prev3Price(s,y)) + 
             s.addedCapacities[y - 1] + s.capacities[y - 1]) ]

// simpler for Clean energy
[getMaxCapacity(s:InfiniteSupplier,y:Year) : Energy
   -> capacity(s,y,prev3Price(s,y))  ]

// CCEM v6 modification : cMax is the current max capacity with the historic price
// for a lower price, we may stop the "expensive" reserve
// let's drop the quadratic term (OCCAM)
[getOutput(s:FiniteSupplier,p:Price,cMax:Energy,y:Year) : float
  -> let cMaxDyn := min(cMax, capacity(s,y,p)),
         cProd := s.production * min(1.0, (cMaxDyn / s.capacityMax)),  // projected output considering capacity 
         pRatio := p / s.price,                                     // relative price vs p0
          f1 := min(cMax, max(0.0, cProd * (1 + (pRatio - 1) * s.sensitivity) * pRatio))  in
        f1]  
...
// linear regression forecast of gw5 creates instability (cf h22)
[prev3Price5(x:Supplier,y:Year) : Price
   -> if (y = 2) x.price 
      else if (y = 3) (x.sellPrices[y - 1] + 2 * x.price) / 3.0
      else (x.sellPrices[y - 1] + x.sellPrices[y - 2] + x.sellPrices[y - 3]) / 3.0 ]
...
[capacity(x:FiniteSupplier,y:Year,p:Price) : Energy   
   -> let prev := prevMaxCapacity(x,y),
          I1 := reserve(x,p),      // inventory with that price, minus gone
        ...
        min(prev * (1.0 + rGrowth),  inventoryToMaxCapacity(x,I1))]

[reserve(x:FiniteSupplier,p:Price) : Energy   
   -> get(x.inventory,p) - x.gone]

// new in CCEM v6: the adaptation to low inventory is piecewise linear (to avoid oscillation)
[inventoryToMaxCapacity(x:FiniteSupplier,inventory:Energy) : Energy
   -> let ratio  := (inventory / x.threshold) in
        min(ratio * x.capacityMax * 140%,
            x.capacityMax * (1.0 + (ratio - 1.0) * 0.5))]
...
// Invariant MaxCapacity(x:Supplier,y:Year) = x.capacities[y] + x.addedCapacities[y]
[getProd(s:Supplier,y:Year) : void
  -> let cMax := getMaxCapacity(s,y), p0 := prev3Price(s,y) in
      (s.capacities[y] := cMax - s.addedCapacities[y - 1],
       if (s = TESTO) (case s (FiniteSupplier trace(SHOW1, "~S capacity = ~F2 (prev:~F2) cMax:~F2 (reserve[~F2]:~F2/~F2)\n",
                               s,s.capacities[y],s.capacities[y - 1],p0,reserve(s,p0),s.threshold))),
...
[prodShow(s:Supplier,y:Year,cMax:Energy) : void
  -> printf("[~A] compute max prod(~S) cMax=~F2(~F2 - adds) @ avgprice:~F2)\n",year!(y),s,s.capacities[y],cMax,prev3Price(s,y)),
...
[showNeeds(c:Consumer,c2:Energy,b:Block,y:Year) : void
  -> printf("[~S] ~S needs = ~F2 (economy ~F%, export ~F%, import ~F%)\n",
...
// OBSOLETE getNeed is the old function that stored all values od HowMuch
...
// [5] this is where we apply the different s-curves to find what the actual consumption is at a given price
...
[recordCapacity(s:Supplier,y:Year) : void
   -> let p1 := s.sellPrices[y], p2 := prev3Price(s,y),
        ...
        (//(s = TESTE) printf(">>>>> ~I",showOutput(s,y,p1)),
         ...
         //(s = TESTO) printf(">>>>> inventory[~A] = ~F2 at price ~F2\n",year!(y),s.inventories[y],p2),
...
// TEMPORARTY test disolve after calling solve
[TESTsolve(pb:Problem, s:Supplier) : Price
  -> if DI disolve(pb,s)
     else let p1 := solve(pb,s), s1 := abs(tryPrice(pb,s,p1)),
         p2 := disolve(pb,s), s2 := abs(tryPrice(pb,s,p2)) in
           (printf("=======[~A] solve(~S) = ~F2 [~F2] vs disolve = ~F2 [~F2]\n",year!(pb.year),s,p1,s1,p2,s2),
            if (s1 < s2) error("TESTsolve : stop and look at ~S",s),
            p1) ]
        ...
        (for i in (1 .. NIS)
           let x := pb.priceRange[i],
              ...
              (assert(p.prodCurve[i] >= 0.0),
               ...
               if (abs(v) < v0) (v0 := abs(v), p0 := x, i0 := i)),
        //[SHOW2] solve(~S) -> price = ~A : delta = ~F2, qty = ~F2  // s, p0,v0,p.prodCurve[i0],
        //(s = TESTE) solveShow(s,p0,v0,i0),
        if (p0 = 0.0) marketModelError(s,y),
        p0) ]



// debug: show the results of the solve
[solveShow(s:Supplier,p0:Price,v0:Energy,i0:integer) : void
...
[saves(c:Consumer,s:Supplier,y:Year,w:Percent) : void
   ->  let i := s.index,
         ...
         (//[DEBUG] [~A] ~S saves ~S% of ~S (was ~S%) --- // year!(y),c,w2,s,w1,
          ...
          c.savings[y][i] := w2,                             // [2] record all savings
...
// (1) versus C02 with a linear interpolation
// (2) using pain levels for economy, energy and climate
[computeSatisfaction(c:Consumer,y:Year) : Percent
  -> let strat := c.objective, 
         ...
         sat1 := 1.0 - (abs(pb.earth.co2Levels[y] - co2Target) / co2Target), 
         sat2 := max(0.0, 1.0 - c.painResults[y]),
         sat3 := max(0.0, 1.0 - c.painEnergy[y]),
         sat4 :=  max(0.0, 1.0 - c.painWarming[y]),
         sat := (strat.weightCO2 * sat1 + strat.weightEconomy * sat2 + 
                 strat.weightEnergy * sat3 + strat.weightDamages * sat4) in
        (//[SHOW5] --- satisfaction for ~S is ~F% // c, sat,
         trace(5,"satisfaction(~S) = ~F% from (~F%:~F1ppm,~F%:~F1,~F%:~F1,~F%:~F1)\n", c,sat,
               sat1,pb.earth.co2Levels[y],sat2,c.painResults[y],sat3,c.painEnergy[y],sat4,c.painWarming[y]),
         sat) ] 
...
[showMaxCapacity(x:FiniteSupplier,y:Year,cMax:Energy,p:Price) : void
  -> let prev := prevMaxCapacity(x,y),
         ...
         c := min(prev * (1.0 + rGrowth), inventoryToMaxCapacity(x,I1)) in
   (printf("[~A] >>> inventory(~S) = ~F2 = ~F2(~F2$) - ~F2 (gone)\n",year!(y),x,I1,get(x.inventory,p),p,x.gone),
    printf("[~A] >>> max capacity(~S@~F2)=~F2->(~F2/~F2) (inventory ratio: ~F% & rProd = ~F% => rGrowth=~F%) Gtep {was:~F2}\n",
            year!(y),x,p,prev,c,cMax,(I1 / x.threshold), rProd, rGrowth,prev)) ]
