...
Version:float :: 0.6            // started on Auhust 12th, 2024
...
Transition <: object(
  ...
  to:Supplier,                   // to s1
  heat%:Percent,                 // part of the energy that is  still used for direct heat vs electricty
  tag:string                     // when we want to print
)

// create a transition (used in test.cl)
[makeTransition(name:string,fromIndex:integer,toIndex:integer,h%:Percent) : void
...
Consumer  <: thing(
    ...
    productivityFactor:Percent = 0%,   // productivity factor (decline as pain grows)
...
WorldClass <: thing(
   ...
   // two factors that show the impact of trade barriers (they could be zone dependent in the future)
...
//           sat(c,y) = weightCO2 * absR(co2(y) - target(y)) + weightGPD * EconomyPain +
//                      weightEnergy * EnergyPain(y) + weightDamages * ClimatePain(y)) }
Strategy <: object(
  stratFrom:Consumer,
  ...
  weightEnergy:Percent,
  weightDamages:Percent             // the rest is assigned to damages pain
)
...
[strategy(tCO2:float,wCO2:Percent,wEconomy:Percent,wEnergy:Percent) : Strategy
  -> Strategy(targetCO2 = tCO2, weightCO2 = wCO2, 
              weightEconomy = wEconomy, weightEnergy = wEnergy, 
              weightDamages = 1 - (wEconomy + wEnergy + wCO2)) ]

// prints a strategy
[self_print(x:Strategy) : void
  -> printf("strategy(CO2:~F1x~F%,Energy:~F%,Economy:~F%,Climate:~F%)",
            x.targetCO2,x.weightCO2,x.weightEnergy,x.weightEconomy,x.weightDamages) ]


// returns the satisfaction score once the satisfaction vector is computed
[satScore(c:Consumer) : float
  -> let s := 0.0, s2 := 0.0, n := pb.year - 1 in 
      (for y in (2 .. (n + 1)) 
...
// the first type of CNU is a cone that generates an affine function
KNUcone <: KNU(
  median:Affine,
  lower:Affine,
  higher:Affine)
