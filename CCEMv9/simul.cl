...
[run(p:Problem, talk?:boolean) : void
  -> let y := p.year + 1 in
       (pb.year := y,
        ...
        for s in Supplier
           (//[SHOW2] ********* energy ~S : ~F2 PWh ********************************************* // s,sum(list{c.economy.needs[y][s.index] | c in Consumer}),
...
[avgTax(s:Supplier,y:Year) : float
 -> let w1 := 0.0, w2 := 0.0 in
      (for c in Consumer
         (w1 :+ tax(c,s,y) * c.economy.needs[y][s.index],
          w2 :+ c.economy.needs[y][s.index]),
       w1 / w2)]
...
/* found duplicate code : try to use the game.cl version instead
...
[carbonTax(y:Year) : float
  -> sum(list{c.economy.carbonTaxAmounts[y] | c in Consumer}) ]
...
[getCo2KWh(c:Consumer,y:Year) : float
  -> sum(list{ (s.co2Kwh * c.consos[y][s.index]) | s in Supplier} ) / 
     sum(list{c.consos[y][s.index] | s in Supplier}) ]
...
[updateChartsConsumer(c:Consumer,y:Year) : void
  -> for s in Supplier
     ...
     udapdateChart(c.charts.needs,y, list<float>{sumNeeds(c,i) | i in (1 .. y)}),
     ...
     udapdateChart(c.charts.carbonTaxAmounts,y, c.economy.carbonTaxAmounts),
...
[see(c:Consumer,y:Year) : void
  -> printf("~S: conso:~F2PWh ~Ivs need:~F2 ~I, elec:~F2\n",c,sum(c.consos[y]),pl2(c.consos[y]),
            sum(c.economy.needs[y]),pl2(c.economy.needs[y]),c.ePWhs[y]) ]
...
[makeChartsConsumer(y:Year) : ChartsConsumer
  -> ChartsConsumer( consos = list<Tmeasure>{makeTmeasure(y) | s in Supplier},
                     ...
                     carbonTaxAmounts = makeTmeasure(y),
...
[init(c:Consumer) : void
  -> for s in Supplier 
     ...
     c.population := list<float>{0.0 | i in (1 .. NIT)},
     c.population[1] := get(c.populationEstimate,yearF(1)),
     c.youngs := list<float>{0.0 | i in (1 .. NIT)},
     c.youngs[1] := get(c.populationEstimate,yearF(1)) * c.populationDistribution[1],
     c.seniors := list<float>{0.0 | i in (1 .. NIT)},
     c.seniors[1] := get(c.populationEstimate,yearF(1)) * c.populationDistribution[3],
     initAdapt(c),
     initBlock(c)]
...
[initBlock(c:Consumer)
   -> let w := c.economy in
         (init(w),
          w.needs := list<list<Energy>>{ list<Energy>() | i in (1 .. NIT)},
          w.needs[1] := c.consumes,
          w.carbonTaxAmounts := list<Price>{0.0 | i in (1 .. NIT)}, 
          ...
          w.giniLevels := list<float>{0.0 | i in (1 .. NIT)},
          w.giniLevels[1] := w.giniStart,
          w.socialExpenseValues := list<Percent>{0.0 | i in (1 .. NIT)},
          w.socialExpenseValues[1] := get(w.socialExpenseRatio, yearF(1))) ]
