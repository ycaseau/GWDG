...
// ********************************************************************
// *    Part 1: KNU cones                                             *
// *    Part 2: Excel Utilities                     *
// *    Part 3: Sensitivity Experiments                               *
// *    Part 4: Three scenarios : Nordhaus / Jancovici / Singularity  *
// *    Part 5: CCEMv7 Tactical Experiments                           *
// *    Part 6: Tactical Optimization (hand tuning version)           *
// *    Part 7: Miscelaneous (go)                                     *
// ********************************************************************
...
// lost somewhere 
[totalConso(s:Supplier, y1:Year, y2:Year) : Energy
  -> let v := 0.0 in
       (for y in (y1 .. y2) v :+ pb.world.all.totalConsos[y],
        v) ]
...
// NOTE: the following KNUs will be used to generate experiments via randomization in CCEM v8
...
// ********************************************************************
// *    Part 2: Excel Utilities &  Callback for CLSERVE               *
// ********************************************************************

// gdp table : with and without inflation  2010 to 2050
[gdpTable() : void
  -> let p := fopen("excel/" /+ "GDP2050" /+ ".csv","w"),
         lyears := list{(1 + (i - 1) * 10) | i in (1 .. 5)} in
    (use_as_output(p),
     printf(",~I\n",
        pListNumber(list{year!(i) | i in lyears})),
     // print GDP with inflation
     for c in Consumer
         printf("~S, ~I \n",c,
                 pListNumber( list{integer!(gdp$(c.economy, i)) | i in lyears})),
      printf("Total (T$ cur.), ~I \n",
         pListNumber(
            list{integer!(sum(list{gdp$(c.economy, i) | c in Consumer})) | i in lyears})),
     // print GDP without inflation
     printf(",~I\n",
        pListNumber(list{year!(i) | i in lyears})),
     for c in Consumer
         printf("~S, ~I \n",c,
                 pListNumber( list{integer!(c.economy.results[i]) | i in lyears})),
      printf("Total (2010 T$), ~I \n",
         pListNumber(
            list{integer!(sum(list{c.economy.results[i] | c in Consumer})) | i in lyears})),
      fclose(p))]

// energy production 2010 to 2100
[energyTable() : void
  -> let p := fopen("excel/" /+ "Energy-" /+ string!(integer!(EU.tactic.transitionStart * 100.0)) /+ ".csv","w"),
         lyears := list{(1 + (i - 1) * 10) | i in (1 .. 10)} in
    (use_as_output(p),
     printf(",~I\n",
        pListNumber(list{year!(i) | i in lyears})),
     ...
     for s in Supplier
         printf("~S, ~I \n",s,
                 pListNumber( list{integer!(s.outputs[i]) | i in lyears})),
      printf("Total (PWh), ~I \n",
         pListNumber(
            list{integer!(sum(list{s.outputs[i] | s in Supplier})) | i in lyears})),
      fclose(p))]
...
// note : the default scenario has no carbon tax 

// recalibration (January) for Model 8 on GDP x Energy
// defau(2020) PNB: 73.7T$, 148.3PWh -> 411.2ppm CO2, 14.8C, 9.9PWh clean, 18.7% electricity// defau(2030) PNB: 84.8T$, 157.1PWh -> 435.6ppm CO2, 15.1C, 14.8PWh clean, 21.7% electricity
// defau(2030) PNB: 84.8T$, 156.1PWh -> 435.7ppm CO2, 15.1C, 14.9PWh clean, 21.7% electricity
// defau(2050) PNB: 104.9T$, 163.4PWh -> 485.4ppm CO2, 15.7C, 20.1PWh clean, 25.0% electricity
// defau(2100) PNB: 137.2T$, 146.6PWh -> 602.6ppm CO2, 16.8C, 25.1PWh clean, 31.8% electricity

// ---- prev : CCEM v7 (much less fossil fuel - but more transition)
...
// h1g: (2100) PNB: 149.8T$, 147.1PWh -> 575.0ppm CO2, 16.6C, 38.9PWh clean, 39.4% electricity
// default for Green capacity : affine(list(2000,0.2), list(2020,0.2),list(2030,1.5),list(2040,2.0),list(2100,4.0)),
h1+ :: Experiment(
   comment = "h1+: Green max capacity growth",
   init = (n){ 
       XtoClean(20%),
...
// h1-: (2100) PNB: 135.1T$, 148.7PWh -> 612.1ppm CO2, 16.8C, 21.4PWh clean, 29.7% electricity
// compare the actual capacity in 2050 versus what is predicted from the max [web.cl]
// pb.clean.outputs[41])  versus maxCleanGrowth(41) => cleanGrowthRate(41) must be less than 100
h1- :: Experiment(
   comment = "h1-: Green limited capacity growth",
   init = (n){ 
       XtoClean(-20%),
       Clean.growthPotential := affine(list(2000,0.2), list(2020,0.6),list(2050,1.0),list(2100,1.5))})
...
// h1c: Coal max capacity growth - no effect in version 8
h1c :: Experiment(
   comment = "h1c: Coal max capacity growth " /+ string%(2%),
   init = (n){ 
       Coal.capacityGrowth := 5%,
       Coal.inventory := improve%(Coal.inventory, 20%)})  // lots of coal


// H2 looks at the sensitivity to fossile inventory - 
// h2- is derived (CCEM v9) from the proven 2020 reserves
// impact on GDP is brutal : 2100 = 2020 levels (+2.5C)
// h2-: (2100) PNB: 75.1T$, 55.3PWh -> 544.5ppm CO2, 16.4C, 14.6PWh clean, 41.6% electricity
h2- :: Experiment(
   comment = "h2-: conservative estimate of Oil inventory - what we know in 2020",
   init = (n){ 
      Oil.inventory := affine(list(30.0, 1545.0),list(60.0,2545.0), list(120.0,3445.0)),     
      Gas.inventory := affine(list(7.0, 1875.0),list(15.0,2455.0), list(30.0,2455.0)),     // 
      Coal.inventory := affine(list(7.0, 6440.0),list(15.0,8940.0), list(30.0,10000.0))})  // 

// h2+ is the sum of proven 2020 reserves and GPT estimates of undiscovered reserves (CCEM v9)
// h2+: (2100) PNB: 228.6T$, 217.8PWh -> 652.9ppm CO2, 17.0C, 36.0PWh clean, 31.1% electricity
h2+ :: Experiment(
   comment = "h2+: more oil to be found at higher price, adding undiscovered reserves",
   init = (n){ 
         Oil.inventory := affine(list(30.0, 2045.0),list(60.0,5545.0), list(120.0,7545.0)),     
         Gas.inventory := affine(list(7.0, 2355.0),list(15.0,7355.0), list(30.0,10355.0)),     
         Coal.inventory := affine(list(7.0, 8840.0),list(15.0,15400.0), list(30.0,25440.0))})  

    

// debug scenario : lots of fossile energy
// marginally higher than h2+
h2++ :: Experiment(
   comment = "h2++: /!\\ DEBUG SCENARIO with plenty of fossile energy",
   init = (n){ 
        Oil.inventory := affine(list(30.0, 3000.0),list(60.0,7000.0), list(120.0,12000.0)),     
        Gas.inventory := affine(list(7.0, 2500.0),list(15.0,9000.0), list(30.0,15000.0)),    
        Coal.inventory := affine(list(7.0, 8500.0),list(15.0,15000.0), list(30.0,30000.0)) })


// h3a: allows extra efficiency investments
// h3a: (2100) PNB: 144.7T$, 125.3PWh -> 586.3ppm CO2, 16.7C, 21.1PWh clean, 31.5% electricity
h3a :: Experiment(
...
// h3b: (2100) PNB: 137.2T$, 146.6PWh -> 602.6ppm CO2, 16.8C, 25.1PWh clean, 31.8% electricity
h3b :: Experiment(
...
// h3c: (2100) PNB: 135.6T$, 145.9PWh -> 601.0ppm CO2, 16.8C, 24.5PWh clean, 32.0% electricity
h3c :: Experiment(
...
// h3d: (2100) PNB: 135.5T$, 145.9PWh -> 601.0ppm CO2, 16.8C, 24.5PWh clean, 32.0% electricity
h3d :: Experiment(
   comment = "h3d: pain reduces fertility and increases early deaths",
   init = (n){ 
        for c in Consumer c.populationFactor := 20% })

// play with sobriety : h3e is a more sober scenario where the consumers are more energy conscious
// check share that is saved using avgSobriety%() -> 5%
// the result is that a little bit of sobriety is good for the economy
h3e :: Experiment(
   comment = "h3e: more enerygy sobriety",
   init = (n){ 
       for c in Consumer c.tactic.cancelFromPain := 100%})
...
// h4-: (2100) PNB: 132.8T$, 147.8PWh -> 612.6ppm CO2, 16.8C, 19.4PWh clean, 28.7% electricity
h4- :: Experiment(
...
// h4+: (2100) PNB: 150.9T$, 148.6PWh -> 579.6ppm CO2, 16.6C, 39.2PWh clean, 39.3% electricity
...
// h4r reduces the subtitution rate if the price is not right (e1 -> e2 if e2 cheaper)
h4r :: Experiment(
   comment = "h4r: less substitution if price is not right",
   init = (n){ pb.transferPriceSensitivity := 100% })
...
// h5-: (2100) PNB: 136.9T$, 148.1PWh -> 603.6ppm CO2, 16.8C, 25.0PWh clean, 32.1% electricity
h5- :: Experiment(
...
//h5+: (2100) PNB: 132.7T$, 141.8PWh -> 598.4ppm CO2, 16.7C, 24.0PWh clean, 32.1% electricity
h5+ :: Experiment(
...
// h5ai : reduces the AI replacement factor
h5ai :: Experiment(
   comment = "h5a: AI replacement is slower",
   init = (n){ for c in Consumer c.economy.aiReplaceFactor := 10% })
...
// h6-: (2100) PNB: 120.8T$, 166.2PWh -> 627.1ppm CO2, 16.9C, 31.3PWh clean, 34.0% electricity
h6- :: Experiment(
...
// h6+: (2100) PNB: 145.4T$, 127.2PWh -> 564.8ppm CO2, 16.6C, 21.0PWh clean, 31.7% electricity
h6+ :: Experiment(
...
// h7+: (2100) PNB: 187.2T$, 189.6PWh -> 659.3ppm CO2, 17.0C, 37.0PWh clean, 34.2% electricity
h7+ :: Experiment(
   comment = "h7+: optimistic outlook on growth",
   init = (n){ 
       US.economy.roiEfficiency := improve(US.economy.roiEfficiency,20%),
       EU.economy.roiEfficiency := improve(EU.economy.roiEfficiency,20%),
       CN.economy.roiEfficiency := improve(CN.economy.roiEfficiency,20%),
       IN.economy.roiEfficiency := improve(IN.economy.roiEfficiency,20%),
       Rest.economy.roiEfficiency := improve(Rest.economy.roiEfficiency,20%)})
...
// h7-: (2100) PNB: 91.6T$, 100.9PWh -> 561.1ppm CO2, 16.6C, 16.5PWh clean, 31.0% electricity
h7- :: Experiment(
...
// h8-: (2100) PNB: 136.0T$, 145.4PWh -> 600.9ppm CO2, 16.8C, 24.8PWh clean, 32.0% electricity
// small decline of EU GDP, 
h8- :: Experiment(
...
// h8: t(2100) PNB: 111.3T$, 114.3PWh -> 569.8ppm CO2, 16.6C, 20.7PWh clean, 32.4% electricity
h8 :: Experiment(
...
// h8+: (2100) PNB: 78.7T$, 72.9PWh -> 524.4ppm CO2, 16.1C, 16.0PWh clean, 34.7% electricity
h8+ :: Experiment(
   comment = "h8+: heavy carbon tax !",
   init = (n){ 
       US.carbonTax := affine(list(380,50.0),list(430,100.0),list(480,200.0), list(600,300.0)),
       EU.carbonTax := US.carbonTax,
       CN.carbonTax := US.carbonTax,
       IN.carbonTax := US.carbonTax,
       Rest.carbonTax := US.carbonTax })
...
// h9d: (2100) PNB: 149.9T$, 159.0PWh -> 610.9ppm CO2, 16.8C, 27.7PWh clean, 32.1% electricity
// cool : default to h9 = environ 6% de GDP en moins 137 -> 149
// defau(2100) PNB: 160.2T$, 130.5PWh -> 623.3ppm CO2, 16.9C, 28.9PWh clean, 36.8% electricity
// h9d: (2100) PNB: 167.2T$, 131.8PWh -> 630.0ppm CO2, 16.9C, 30.7PWh clean, 38.1% electricity
h9d :: Experiment(
...
// Nordhaus like impact (approx 3%)
// h9-: (2100) PNB: 142.9T$, 150.3PWh -> 604.8ppm CO2, 16.8C, 26.0PWh clean, 32.0% electricity
h9- :: Experiment(
...
// h9+: (2100) PNB: 155.3T$, 130.7PWh -> 621.3ppm CO2, 16.9C, 27.1PWh clean, 35.5% electricityq
// this example shows some rebound effect for China (destruction -> less demand -> lower prices -> better for CN (relatively))
h9+ :: Experiment(
...
// h9++:(2100) PNB: 149.7T$, 131.6PWh -> 617.9ppm CO2, 16.8C, 25.3PWh clean, 34.0% electricity
h9++ :: Experiment(
   comment = "h9++: Global warming damages with high values for impact",
   init = (n){ 
...
// this is better tested with tactic optimization loop
// adapt(2100) PNB: 131.6T$, 141.8PWh -> 591.3ppm CO2, 16.7C, 23.7PWh clean, 31.9% electricity
h10t :: Experiment(
   comment = "adaptation for everyone",
   init = (n){ for c in Consumer c.tactic.adaptStart := 30% })

// h10-:(2100) PNB: 127.5T$, 137.6PWh -> 586.7ppm CO2, 16.7C, 22.9PWh clean, 31.8% electricity
h10- :: Experiment(
   comment = "h10-: Adaptation is more expensive and difficult",
   init = (n){ 
       for c in Consumer 
          (c.tactic.adaptStart := 30%,
           c.adapt := Adaptation(efficiency = affine(list(0,0.0), list(700%,50%))))})

// h10+:(2100) PNB: 129.2T$, 139.2PWh -> 587.7ppm CO2, 16.7C, 23.2PWh clean, 31.8% electricity
h10+ :: Experiment(
...
// h11: (2100) PNB: 134.5T$, 145.0PWh -> 599.7ppm CO2, 16.7C, 24.3PWh clean, 32.0% electricity
// EU = 9.03 -> 8.27
h11 :: Experiment(
...
// this is done by combining one the experiment context : go(h*) and topt(c:Consumer)
// test with c = EU, US and CN
// Gaia.painDelay := 10 => way to test the benefits of anticipation



// ********************************************************************
// *    Part 6: Miscelaneous (go)                                     *
// ********************************************************************
