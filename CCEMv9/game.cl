...
// *       file: game.cl                                              *
// ********************************************************************
// @context: refer to resources/claire-language-reference.md for syntax and semantics
...
// this code is instrumented for debug: three control variables for traces
...
// debug new features
CCEMv9:integer :: 5    // debug variable : set to 1 to use new features (v9) - default is 0
// when done, remove and replace CCEMV9 with SHOWX where X is the model number

// ********************************************************************
// *    Part 1: Production model M1                                   *
// ********************************************************************

SHOW1:integer :: 1        // verbosity for model M1 
...
[getMaxCapacity(s:FiniteSupplier,y:Year) : Energy
  ->  expectedCapacity(s,y)  ]     //   CCEM v7 fsh(expectedCapacity(s,y) + s.capacities[y - 1]) / 2 ]
...
[getSupply(s:Supplier,p:Price,cMax:Energy,y:Year) : float
  -> let cProd := s.production * (cMax / s.capacityOrigin),  // projected output considering capacity 
         pRatio := p / get(s.equilibriumPrice,yearF(y)) in                                     // relative price vs p0
           min(cMax, cProd * pRatio)]

// what we expect from previous year: last year capacity + added capacity during year y (decision of y-1)
[forwardMaxCapacity(x:Supplier,y:Year) : Energy
...
[expectedCapacity(x:FiniteSupplier,y:Year) : Energy   
   -> let prev := forwardMaxCapacity(x,y),
...
[prodGrowth(x:Supplier, prev:Energy, y:Year) : Energy
  ->  if (y <= 3) 2%  // first 3 years, we assume a 2% growth 
      else let s := (7 * x.rawNeeds[y - 1] -  2 * x.rawNeeds[y - 2] - x.rawNeeds[y - 3]) / 4 in
...
[prodInspect(x:Supplier, prev:Energy, s:Energy, y:Year) : void
  -> printf("[~A] >>> prev(~S)=~F2, 3 years is ~F2 Gtoe from ~A\n",year!(y),x,prev,s, 
                    list{x.rawNeeds[i] | i in (max(2,y - 3) .. y - 1)})]
...
[computeCapacity(s:Supplier,y:Year) : void
  -> let cMax := getMaxCapacity(s,y) in
      (s.capacities[y] := cMax,    /// was : - s.addedCapacities[y - 1],
       s.capacityMax :max cMax,
      //(s = TESTO) showMaxCapacity(s,y,cMax) 
      )]  
...
[getNeed(c:Consumer,y:Year) : void
  -> let b := c.economy,
         c0 := sum(c.consumes),              // v9: sum of "start needs" at year 1
         dmr := dematerializationRate(c,y),
         c2 := c0 * dmr * globalEconomyRatio(b,y) * (1.0 - b.disasterRatios[y - 1]) in   
      (//[SHOW2] === needs(~S) = ~F2 PWh vs ~F2 (gdp ratio:~F%) // c,c2,c0,globalEconomyRatio(b,y),
       ...
       b.needs[y] := list<Energy>{ (c2 * ratio(c,s)) | s in Supplier},
       if (y > 1)   // [4] execute transfers from subsitution capabilities setup at year y - 1
          for tr in pb.transitions transferNeed(c,y,tr,transferRate(  c,tr,y - 1) * b.needs[y][tr.from.index]),
       // v0.9: update the population subsegments
       getPopulation(c,y))]


// debug: show the needs
[showNeeds(c:Consumer,c2:Energy,b:Block,y:Year) : void
  -> trace(1,"--- globalEconomyRatio(b,y) = ~F3 * ~F3\n",economyRatio(b,y), tradeRatio(b,y)),
     trace(1,"--- economyRatio(b,y) = ~F3 / ~F3\n",newMaxout(b,y),b.gdp),
     showMaxout(b,y),
     showRoi(b,y),
...
[dematerializationRate(c:Consumer,y:Year) : Percent
  -> (1 - get(c.economy.dematerialize,yearF(y))) * (1 - c.savingRates[y - 1]) ]
...
[populationEstimate(c:Consumer,y:Year) : float
...
[transferNeed(c:Consumer,y:Year,tr:Transition,q:Energy) : void
  -> //(tr.from = TESTE | tr.to = TESTE) trace(0,">>>> Need transfer of ~F2 PWh for ~S from ~S to ~S [~F%]\n",q,c,tr.from,tr.to,tr.efficiency%),
     c.economy.needs[y][tr.from.index] :- q,
     c.economy.needs[y][tr.to.index] :+ q * tr.efficiency%,
...
[oilEquivalent(s:Supplier,p:Price) : Price
  -> p * pb.oil.sellPrices[1] / s.sellPrices[1] ] 
...
[howMuch(c:Consumer,s:Supplier,p:Price) : Energy
  -> let cneed := c.economy.needs[pb.year][s.index], x1 := getCancel(c,s,p), 
         x := max(0.0,(1.0 - x1)) in
...
[recordCapacity(s:Supplier,y:Year) : void
   -> let p1 := averagePrice(s,y,3),          // was s.sellPrices[y] in CCEM v8
          addCapacity := max(0.0,s.capacities[y] - forwardMaxCapacity(s,y)) in
        (//(s = TESTO) printf(">>>>> ~I",showOutput(s,y,s.capacities[y],p1)),
         (case s (FiniteSupplier s.inventories[y] := get(s.inventory,p1) - s.gone)),
         //(s = TESTO & s % FiniteSupplier) printf(">>>>> inventory[~A] = ~F2 at price ~F2 avg\n",year!(y),s.inventories[y],p1),
...
[averagePrice(s:Supplier,y:Year,n:integer) : Price
  -> let n1 := max(1,y - n + 1), n2 := y, 
         sum := sum(list{ s.sellPrices[i] | i in (n1 .. n2)}) in
        (sum / float!(n2 - n1 + 1)) ]
...
[getPopulation(c:Consumer,y:Year) : void
  -> let pop := get(c.populationEstimate,yearF(y)),
         ...
         births := (pop - popPrev) + popPrev * deathRate,
         activePrev := popPrev - c.youngs[y - 1] - c.seniors[y - 1] in
      (trace(CCEMv9,"[~A] ~S pop (~F3->~F3) active = ~F2, births: ~F2M =~F2+~F2\n",
             year!(y),c,popPrev,pop,activePrev,births * 1000.0,
              (pop - popPrev) * 1000.0,(popPrev * deathRate * 1000.0)),
       c.youngs[y] := (19.0 / 20.0) * c.youngs[y - 1] + births,
       c.seniors[y] := (1.0 / 40.0) * activePrev * (1.0 - deathRate) +
                        c.seniors[y - 1] * (1.0 - deathRate) * pb.seniorExcessMortality,
       trace(CCEMv9,"[~A] ~S population ~F2 -> youngs:~F2 seniors:~F2(~F2+~F2), active:~F2\n",
            year!(y),c,pop,c.youngs[y],c.seniors[y],
            (1.0 / 40.0) * activePrev * (1.0 - deathRate),
            c.seniors[y - 1] * (1.0 - deathRate) * pb.seniorExcessMortality,
            pop - c.youngs[y] - c.seniors[y])) ]
   
// active population ratio is used to compute social costs (M5)
[activePop%(c:Consumer,y:Year) : Percent
  -> let pop := get(c.populationEstimate,yearF(y)),
         active := pop - c.youngs[y] - c.seniors[y] in
        (active / pop) ]  

// debug: show the population distribution
[showPop(c:Consumer,y:Year) : void
  -> let pop := get(c.populationEstimate,yearF(y)),
         active := pop - c.youngs[y] - c.seniors[y] in
        trace(1,"[~A] ~S population = ~F2 (youngs:~F2 seniors:~F2 active:~F2)\n",
              year!(y),c,pop,c.youngs[y],c.seniors[y],active) ]

// average age heuristics (see Excel Model)
[averageAge(c:Consumer,y:Year) : float
  -> let pop := get(c.populationEstimate,yearF(y)),
         active := pop - c.youngs[y] - c.seniors[y] in
        ((c.youngs[y] * 10.0) + (c.seniors[y] * 75.0) + (active * 40.0)) / pop  ]
...
[record(c:Consumer,s:Supplier,y:Year)
  -> let i := s.index,
       ...
       (if (s = TESTO) trace(5,"DEBUG [~A] cancel(~S,~S) is ~F% (at oep = ~F2) versus ~F%(x10)\n",
        ...
        trace(SHOW3,"=== sell price for ~S = ~F2\n",s,p),
        saves(c,s,y),   
        ...
        s.rawNeeds[y] :+ cneed,                            // "real" needs before substitution
...
[cancels(c:Consumer,s:Supplier,y:Year,x:Energy) : void
 -> c.economy.cancels[y] :+ x,                                      // sum for all suppliers
    c.cancel%[y][s.index] := x / c.economy.needs[y][s.index]  ]     // record all cancel rates
         
// [4] [6]  consumes : register the CO2 and register the energy
[consumes(c:Consumer,s:Supplier,y:Year,x:Energy) : void
   -> if (s = TESTE) 
          trace(1,"[~A] ~S consumes ~F2 of ~S [need = ~F2 reduced-> ~F2] \n", year!(y),c,x,s,c.economy.needs[y][s.index],howMuch(c,s,truePrice(c,s,y))),
      ...
      c.economy.carbonTaxAmounts[y] :+ tax(c,s,y) * x / 1000.0,      // [4] tax in T$, energy in PWh
...
// CCEM v8: the modulation of c.transitionFactors is only applied post 2020 (TransitionPivot) and is capped
TransitionPivot :: 2020
[getTransferRate(c:Consumer,tr:Transition,y:Year) : Percent
  -> let tr1 := get(c.subMatrix[tr.index],yearF(y)) in
      (if (year!(y) <= TransitionPivot) tr1
       else let tr2 := get(c.subMatrix[tr.index],float!(TransitionPivot)) in
              (tr2 + (tr1 - tr2) * c.transitionFactors[y - 1]))]
...
[updateRate(c:Consumer,s1:Supplier,tr:Transition,y:Year,consumed:Energy) 
   -> let i := tr.index, s2 := tr.to,
          ...
          w1 := transferRate(c,tr,y - 1),                // transfer rate last year
          w2 := max(w1, getTransferRate(c,tr,y)),        // transfer expected for this year (monotonic !)
          w3 := applyMaxGrowthRate(w1,w2,s1,s2,y) in     // modulo capacity growth & price constraints 
         (c.substitutions[y][i] := w1 * consumed,        // actual substitution (rate of previous year)
          ...
          s2.additions[y] :+ (w3 - w1) * consumed,                // ... keep a history of addition for s2
           //(s2 = TESTO) trace(0,"[~A] --- ~S increase transfer ~F1PWh, additions(~S) =~F2\n", year!(y),c,(w3 - w1) * consumed,s2,s2.additions[y]),
          //[SHOW3] [~A] ~S transfers ~F3PWh ~S to ~S, rate=~F% [matrix:~F%,max:~F%] maxFlow:~F2PWh // year!(y),c,w1 * consumed,s1,s2,w3,getTransferRate(c,tr,y),maxTransferRate(s2,y),maxTransferFlow(s2,y),
          //[CCEMv9] [~A] ~S transfers from ~S(~F1) => ~S(~F2) [~F%] // year!(y),c,s1,s1.sellPrices[y - 1],s2, s2.sellPrices[y - 1],priceDeltaRate(s1,s2,y),
          ...
          c.eDeltas[y] :+ (w1 * consumed) * eTransferRatio(c,tr),     // for debug (electrification via transfer)
          //(s2 = TESTE | tr.from = TESTE |c = TESTC) showUpdate(c,s1,s2,tr,y,consumed,w1,w3),
...
[showUpdate(c:Consumer,s1:Supplier,s2:Supplier,tr:Transition,y:Year,consumed:Energy,w1:Energy,w3:Energy) : void
  -> trace(TALK,"[~A] ~S transfer ~F2 PWh(~F%) [~F% now on -> add ~F3] of ~S to ~S [matrix ->~F%]\n",
            year!(y), c,w1 * consumed,w1,w3,(w3 - w1) * consumed, s1,tr.to, 
...
[maxTransferRate(s:FiniteSupplier, y:Year) : Percent
  -> let f := maxTransferFlow(s,y) in
      (if (f > 0.0) min(1.0, (s.capacityGrowth * forwardMaxCapacity(s,pb.year) / f)) else 0.0)] 

// for Clean, s.growthPotential is the max PWh that we can add in a year
[maxTransferRate(s:InfiniteSupplier, y:Year) : Percent
  -> let f := maxTransferFlow(s,y) in 
       (if (f > 0.0) min(1.0, (get(s.growthPotential,yearF(pb.year)) / f)) else 0.0) ]
          ...
          (if (tr.to = s)
             for c in Consumer
                let w1 := transferRate(c,tr,y - 1),
                    w2 := max(w1, getTransferRate(c,tr,y)) in
...
[priceDeltaRate(s1:Supplier,s2:Supplier,y:Year) : Percent
  -> let p1 := s1.sellPrices[y - 1], p2 := s2.sellPrices[y - 1] in
        100% - max(0%, min(100%, ((p2 - p1) / p1) * 2 * pb.transferPriceSensitivity)) ]
...
[getEconomy(y:Year) 
  -> for b in Block checkBalance(b.describes,y),
     ...
     for c in Consumer                // added to CCEMv7: part of cancel that is voluntary
         c.economy.sobriety[y] := c.economy.cancels[y] * (c.cancelAcceleration / (1.0 + c.cancelAcceleration)),
     let e := pb.world.all in
       (consolidate(e,y),
        ...
        //[SHOW4] --- World GDP = ~F2T$ from actual energy ~F1Gtoe, Inv=~F2 (was ~F2) // e.results[y],e.totalConsos[y],e.investGrowth[y],e.investGrowth[y - 1],
        e)]    
...
[newMaxout(b:Block,y:Year) : Price
  -> (b.maxout[y - 1] * (1 - decay(b,y)) * productionDecline(b,y) +   // old value-assets
      b.investGrowth[y - 1] * roI(b,y)) ]                              // new value-assets

// show the maxout growth (without the effects of energy loss)
[showMaxout(b:Block,y:Year) : void
  -> printf("=== MaxOut for block ~S in year ~A is ~F2T$ [+~F%] (decay: ~F2% invest: ~F2T$ roi: ~F2%)\n",
            b,year!(y),newMaxout(b,y),(newMaxout(b,y) - b.maxout[y - 1]) / b.maxout[y - 1],
            decay(b,y),b.investGrowth[y - 1],roI(b,y))]
...
[roI(b:Block,y:Year) : Percent
  -> defaultRoI(b,y) * get(b.roiEfficiency,yearF(y)) *
     getCompetitiveness(b,y) * (1 - b.socialExpenseValues[y - 1])]
...
[defaultRoI(b:Block,y:Year) : Percent
  -> pb.world.returnOnInvestment * (1.0 + b.describes.tactic.roiAdjustment * (min(y,100) / 100.0))]

// debug : showROI
[showRoi(b:Block,y:Year) : void
   -> printf("=== ROI for block ~S in year ~A is ~F2%=~F% * ~F2% * ~F2% * [~F2%:~F2%]\n",b,year!(y),
             roI(b,y),defaultRoI(b,y), get(b.roiEfficiency,yearF(y)),
             (1 - b.socialExpenseValues[y - 1]) ^ 2.0,
             getCompetitiveness(b,y), laborCostFactor(b,y))]

// competitiveness is a KNU (affine) that takes labor cost (proxy : gdp/h) as input
[laborCostFactor(b:Block,y:Year) : Percent
   -> ((b.results[y - 1] / populationEstimate(b.describes,y)) / 
        (pb.world.all.results[y - 1] / worldPopulation(y)))]

// computes the competitiveness of b based on labor cost
[getCompetitiveness(b:Block,y:Year) : Percent
   -> get(pb.world.competitivenessFactor, laborCostFactor(b,y))]


// decay is the rate at which productive assets dissolve, depends on economy maturity
[decay(b:Block,y:Year) : Percent
  -> get(b.decayTable, yearF(y))]
...
[productionDecline(w:Block,y:Year) : Percent
  -> let c := w.describes in 
        (if (y = 2) 1.0 
          else  laborProductionFactor(w,y,(productivityLoss(c,y - 1) / productivityLoss(c,y - 2)))) ]
...
[productivityLoss(c:Consumer,y:Year) : Percent
   -> if (y = 1) 1.0
      else let ymin := max(y - 3, 1), ymax := y - 1,
          avg := sum(list{ c.painLevels[i] | i in (ymin .. ymax) }) / (ymax - ymin + 1) in
      (1.0 - avg * c.productivityFactor)]

// CCEM v9 : labor (active population) is a factor to produce wealth
// [4] differential : one year versus the previous one - look at CCEM v7 function "populationGrowth"
// AI smooth the effect of population evolution both ways: decline of people and decline of productivity because of pain
APFv10:boolean :: false   // hook for v10 => requires precise data mining of active population ratio 
[laborProductionFactor(w:Block,y:Year,decline:Percent) : Percent
  -> let pf := (populationEstimate(w.describes,y) / populationEstimate(w.describes,y - 1)),
         arf := (if APFv10 (activePop%(w.describes,y) / activePop%(w.describes,y - 1)) else 1.0),        // decline of active population ratio 
         aif := w.describes.tactic.aiReplaceFactor in
      (((100% - aif) * pf * arf * decline) + aif)]
...
[consumes(b:Block,y:Year) : void
  -> let e := pb.earth, 
       ...
       (b.disasterRatios[y] := disasterFactor,
        ...
        // if dmatch(b.describes,"EU") showMaxout(b,y),  // debug 
        pb.totalInvest :+ b.investGrowth[y - 1],                          // book keeping for average RoI: (1) invest
        pb.totalGrowth :+ b.investGrowth[y - 1] * roI(b,y),   // book keeping for average RoI: (1) invest
        ...
        //[SHOW4] --- Growth for ~S -> ~F2T$(~F%), invest(~F2) x roi (~F2) -> +~F2T$,  // b, b.maxout[y], ((b.maxout[y] - b.maxout[y - 1]) / b.maxout[y - 1]), iv, roI(b,y), iv * roI(b,y),
        // if dmatch(b.describes,"EU") showRoi(b,y),    // debug : control RoI growth
        //[5] --- temperature is now ~F2, loss is ~F% // t,get(b.describes.disasterLoss,warming(y - 1)),
        ...
        trace(5,"[~A] ~S maxout is ~F2 and result is ~F2 ========\n",year!(y),b,b.maxout[y],b.results[y]),
        trace(5,"[~A] ~S lossRatio ~F%, damage ~F%, and tradeRatio is ~F% ========\n",year!(y),b,b.lossRatios[y],disasterFactor,tradeRatio(b,y)),
...
[computeInvest(b:Block,y:Year) : void
   -> let  iv := b.investGrowth[y - 1],      // last year invest
         ...
         (//[SHOW4] M4: ~S invest ~F% of GDP (grows from ~F2T$ to ~F2T$ = ~F%), target=~F% // b.describes, (iv / r1), r1, r2, (r2 - r1) / r2, b.iRevenue,
          ...
          // CCEM v8: carbon tax reinjected into investment (in v8 we can split between investment and redistribution)
          pb.totalEInvest :+ invE,            // book keeping for total Energy Investx
          b.investGrowth[y] := (ix - invAdapt + b.carbonTaxAmounts[y]),  // CCEM v8 : was (ix - invE - invAdapt)
          if false showGrowth(b,y))]

// show the growth = invest x RoI
[showGrowth(b:Block,y:Year) : void
  -> printf("[~A] ~S growth = ~F%, from invest=~F2T$[~F%] (energy: -~F%)x RoI ~F%\n",
             year!(y),b,(b.results[y] - b.results[y - 1]) / b.results[y - 1],
             b.investGrowth[y],(b.investGrowth[y] / b.results[y]), marginReduction(b.describes,y),
             roI(b,y)) ]
...
[impactFromCancel(b:Block,y:Year) : Percent 
   ->  let s_energy := 0.0, s_cancel := 0.0, s_control := 0.0, c := b.describes,
           ...
           cancel := sumCancelBySupplier(c,y), 
...
[react(e:Earth,y:Year) : void
   -> let x := e.co2Levels[y - 1] in // previous CO2 level
         (e.co2Levels[y] := x + e.co2Emissions[y] * e.co2Ratio,    // simplified in model v5
          ...
          for c in Consumer computeGini(c,y),             // inequality proxy (CCEM v9)
          ...
          for c in Consumer
            let pain_energy := painFromCancel(c,y),       // [3] pain drivers
             ...
             (//[SHOW5] pain for ~S=~F2  ~F2(co2)+ ~F2(cancel)+ ~F2(gdp) // c,pain,get(e.painClimate,get(e.warming,e.co2Levels[y])),painFromCancel(c,y),painFromResults(c,y),
              ...
              c.painResults[y] := (2  * c.painResults[y - 1] + pain_results) / 3.0,    // dampening in v9
              c.painWarming[y] := pain_warming,
              redirection(c,y,(if (y > e.painDelay) c.painLevels[y - e.painDelay] else 0))), // CCEM v8
          computeProtectionism(y),
          computeAdaptation(y)) ]

// [5] computes the redirection for a consumer from pain level
// in CCEM v8, we introduce a delay between the pain lebel, the times it is heard, the time to decide and to act
MAXTAX :: 1000.0
MAXTR :: 150.0     // max transition acceleration compared to best plan
MAXCANCEL :: 3.0   // max cancel acceleration compared to best plan (ratio)
[redirection(c:Consumer,y:Year,pain:Percent) : void
  ->  c.satisfactions[y] := computeSatisfaction(c,y),
      if (year!(y) > THISYEAR)     // CCEM v9 => onfly apply redirection for future years 
        (c.taxAcceleration := MAXTAX * c.tactic.taxFromPain * pain,
         c.cancelAcceleration := MAXCANCEL * c.tactic.cancelFromPain * pain,
...
[taxRate(c:Consumer,y:Year) : Price
  -> let t := c.economy.carbonTaxAmounts[y] in 
...
[economyScale(c:Consumer,y:Year) : float
   -> let w := pb.world, pn := get(c.populationEstimate,yearF(y)), b := c.economy in
     (b.results[y] / pn) * 
...
[painFromResults(c:Consumer,y:Year) : Percent
   -> let w := pb.world.all,
          ...
          growth := (r2 - r1) / r1 in 
        (get(pb.earth.painGrowth,growth) + painFromAIReplacement(c,y)) ]     // new in CCEM v9 : pain from AI replacement of labor

// [7] computes the wheat crops output
Gt2km2 :: 1e-3    // transform m2/MWh into millionskm2/PWh
[agroOutput(y:Year) : void
   -> let w := pb.world, e := pb.earth,
          ...
          efficiencyRatio := get(w.agroEfficiency, avgOilEquivalent(y)) * get(w.bioHealth,warming(y - 1)) * get(w.cropYield,yearF(y)) in
        (//[5] ---- agroPrice ~F2, bioHealth ~F2, cropYield ~F2 // get(w.agroEfficiency, avgOilEquivalent(y)), get(w.bioHealth,warming(y - 1)), get(w.cropYield,yearF(y)),
         ...
         w.agroSurfaces[y] := (w.agroLand - w.energySurfaces[y]) * 
                              get(w.lossLandWarming,warming(y)),
...
// handy
[warming(y:Year) : float
  -> pb.earth.temperatures[y] - pb.earth.avgCentury] 
   ...
   ->  for c in Consumer
        (if (c.adapt.levels[y - 1] < c.adapt.efficiency.maxValue)   // adaptation still has benefits
         ...
         let dommage := c.economy.results[y] * get(c.disasterLoss,3.0),
           ...
           (//[5] [~A] ~S: adaptation ~F2T$ > ~F2T$ for ~S -> ~F% of dommage ~F2 // year!(y),c,c.adapt.spends[y],c.adapt.sums[y],c,investRatio,dommage,
...
[computeGini(c:Consumer,y:Year) : void
  -> // adjust social expenses based on activePop%
     c.economy.socialExpenseValues[y] := get(c.economy.socialExpenseRatio,yearF(y)) *     // init value
        ((100% - pb.activeToSocial) + (pb.activeToSocial * (activePop%(c,1) / activePop%(c,y)) ^ 2.0)),
     //[SHOW5] ~S social expenses for ~A is ~F2 -> ~F2 (activePop% = ~F%) // c,year!(y),get(c.economy.socialExpenseRatio,yearF(y)),c.economy.socialExpenseValues[y],activePop%(c,y),
     // giniLevel as a heuristic function of AI and social expenses
     //[SHOW5] ~S giniLevel for ~S is ~F% modulo socialExpense=~F2,AIreplace=~F% // c,year!(y),c.economy.giniStart,c.economy.socialExpenseValues[y],getAIReplaceFactor(c,y),
     c.economy.giniLevels[y] := c.economy.giniStart * 
        ((100% - pb.socialToGini) + 
             pb.socialToGini * (c.economy.socialExpenseValues[1] / c.economy.socialExpenseValues[y])) * 
        ((100% - pb.aiToGini) + pb.aiToGini * getAIReplaceFactor(c,y)) ]

// pain from AI replacement is added to painFromResults in CCEM v9
[painFromAIReplacement(c:Consumer,y:Year) : Percent
    -> get(pb.earth.painReplacement,getAIReplaceFactor(c,y)) ]
...
[getAIReplaceFactor(c:Consumer,y:Year) : Percent
  -> if (y > 1) c.tactic.aiReplaceFactor * min(1.0, float!(y) / float!(pb.aiTransitionDuration)) else 0.0 ]
...
[dmatch(x:thing,y:string) : boolean
  -> string!(x.name) = y]

// debug : show production and capacity
[showProduction(s:Supplier,y:Year,cMax:Energy) : void
  -> printf("[~A] prodShow(~S) cMax=~F2(adds:~F2); needs=~F2TWh\n",year!(y),s,
            s.capacities[y],s.additions[y - 1],sum(list{c.economy.needs[y][s.index] | c in Consumer}))]

// debug: explain the reasonning of supplier equation (getOutput)
[showOutput(x:Supplier,y:Year,cMax:Energy,p:Price) : void
  -> let cProd := x.production * (cMax / x.capacityOrigin),  // projected output considering capacity
         pRatio := p / get(x.equilibriumPrice, yearF(y)) in         // relative price vs expected
     printf("[~A] output(~S)@~F2=~F2 {max:~F2, projected:~F2} (pratio: ~F%)\n",
            year!(y),x,p,getOutput(x,p,y),cMax,cProd,pRatio)]

// debug: explain the reasonning for max capacity (finite case)
[showMaxCapacity(s:FiniteSupplier,y:Year,cMax:Energy) : void
  -> let prev := s.capacities[y - 1],                   // we want to apply maxDelta to all
         I1 := s.inventories[y - 1],                   // inventory with that price, minus gone
         rProd := prodGrowth(s,prev,y),          // what growth should be
         rGrowth := max(0.0, min(rProd, s.capacityGrowth)),
         c := inventoryToMaxCapacity(s,prev * (1.0 + rGrowth),I1) in
   (printf("[~A] >>> inventory(~S) = ~F2; ~F2 (gone)\n",year!(y),s,I1,s.gone),
    printf("[~A] >>> max capacity(~S)=~F2 -> ~F2 (formula) ->(inventory ratio: ~F% & rProd = ~F% => rGrowth=~F%) Gtep {additions:~F2}\n",
            year!(y),s,prev,cMax,(I1 / s.threshold), rProd, rGrowth,s.additions[y - 1])) ]

// same for InfiniteSupplier
[showMaxCapacity(s:InfiniteSupplier,y:Year,cMax:Energy) : void
  -> let  prev := s.capacities[y - 1],    // we want to apply maxDelta to all
          maxDelta := maxYearlyAdditions(s,y),     // how huch capacity can be added this year
          rProd := prodGrowth(s,prev,y),           // what growth should be
          growth := max(0.0, min(rProd * prev, maxDelta)) in  // cap growth follows conso but is bounded 
         (printf("[~A] >>> max capacity(~S)=~F2->~F2 (rProd=~F%,maxD=~F2 => growth=~F2) PWh {additions:~F2}\n",
               year!(y),s,prev,cMax, rProd, maxDelta, growth, s.additions[y - 1]),
          assert(cMax >= prev + s.additions[y - 1]))]    // transfer decisions from past year are applied
...
[sumNeeds(c:Consumer,y:Year) : float 
   -> sum(c.economy.needs[y])]

[sumConsos(c:Consumer,y:Year) : float 
   -> sum(c.consos[y])]

// v8 : more detailed evaluation of cancel (than c.cancels)  
[sumCancelBySupplier(c:Consumer,y:Year) : Energy
  -> sum(list{ (c.economy.needs[y][s.index] * c.cancel%[y][s.index]) | s in Supplier}) ]
