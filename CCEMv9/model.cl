...
Version:float :: 0.9            // started on July 4th, 2026
ORIGIN:integer :: 2010          // 1980 or 2010  <v8>
THISYEAR:integer :: 2026        // current year for the simulation => when we can apply tactical changes
NIT:integer :: 221              // max number of years in simulation
...
PMIN:Price :: 1.0               // min price for a MWh (to be tuned)
PMAX:Price :: 2500.0            // max price for a  MWh (to be tuned) -> constant ORIGIN dollars
CARNOT:float :: 3.0             // ratio heat to electricity

// we use a relative index that sarts at 1 for 2010 or 1980, based on ORIGIN
year!(i:Year) : integer -> (ORIGIN - 1 + i)
yIndex(i:integer) : Year -> (i + 1 - ORIGIN)
yearF(i:Year) : float -> float!(ORIGIN - 1 + i)
...
[affine(l:listargs) : Affine
  -> make_affine(list<float>{float!(x[1]) | x in l}, list<float>{float!(x[2]) | x in l}) ]
...
[diet_copy(x:ListFunction) : ListFunction
  -> case x (Affine Affine(n = x.n), any StepFunction(n = x.n)) ]

// adds a constant for a ListFunction
[adds(l:ListFunction,x:float) : ListFunction
  -> let l2 := diet_copy(l) in     // key the type StepFunction or Affine
       (l2.xValues := l.xValues, 
        l2.yValues := list<float>{ (l.yValues[i] + x) | i in (1 .. l.n)},
        l2.minValue := l.minValue + x,
        l2.maxValue := l.maxValue + x,
        l2)]

// simple scalar product for a ListFunction
[scalarProduct(l:ListFunction,x:float) : ListFunction
  -> let l2 := diet_copy(l) in     // key the type StepFunction or Affine
       (l2.xValues := l.xValues, 
        l2.yValues := list<float>{ (l.yValues[i] * x) | i in (1 .. l.n)},
        l2.minValue := l.minValue * x,
...
[boundedMultiply(x:float,y:float,minVal:float,maxVal:float) : float
  -> (if (y <= 1.0) minVal + (x - minVal) * y    // linear
      else x + (maxVal - x) * ((y - 1.0) / y))]     // hyperbolic
...
[boundedProduct(l:ListFunction,x:float,minVal:float,maxVal:float) : ListFunction
  -> let factor := boundedMultiply(l.maxValue,x,minVal,maxVal) / l.maxValue in
       scalarProduct(l,factor)]
...
Supplier <: thing(
    ...
    capacityOrigin:Energy,           // the max capacity when we start
    equilibriumPrice:Affine,      // price for 1 MWh, expected equilbrium price (no shortage/surplus)
    ...
    // simulation data
    capacityMax:Energy,           // the max capacity throughout the simulation
    ...
    rawNeeds:list<Energy>,        // keep track of initial needs (without substitution)
...
[makeTransition(name:string,fromIndex:integer,toIndex:integer,h%:Percent,e%:Percent,a%:Percent) : void
 -> let tr := Transition(index = 1 + length(pb.transitions), from = supplier!(fromIndex), 
                         to = supplier!(toIndex), tag = name) in
...
[getTransition(s1:Supplier,s2:Supplier) : Transition
  -> let x := some(tr in s1.from | tr.to = s2) in
    (case x (Transition x, any error("no transition exists from ~S to ~S",s1,s2))) ]

// debug short cut : find a transition through its index
tr!(i:integer) : Transition
  -> pb.transitions[i]
...
Sector <: thing(
    index:integer = 1,                  // index of the sector
    energy%:list<Percent>,              // the weight of the sector in Oil, Coal, Gas and Clean
    subMatrix:list<Affine>               // substitution matrix for energy forms (SETM)
)

// each zone substitution matrix is a linear combination of the SETM with a transition speed
// see simul.cl
...
claire/Consumer  <: thing(
    ...
    populationEstimate:Affine,        // population forecast (in Billions) as a function of years
    population:list<float>,           // population for each year, with warming impact
    populationDistribution:list<Percent>, // distribution of population by age (young, adult, senior)
    adultPopulation:list<float>,      // population between 20 and 60 for each year
    youngs:list<float>,               // population younger than 20 for each year
    seniors:list<float>,              // population older than 60 for each year, with warming impact
    deathRates:Affine,                // death rates (average per 1000) as a function of years
...
Tactics <: object(
   ...
   transitionStart:Percent = 0%,      // minlevel of Energy transition
   transitionFromPain:Percent = 100%,   // accelerate the transition to clean energy
   ...
   aiReplaceFactor:Percent = 0%,       // v9: fraction of AI that replaces human labor (0% to 100%)
   roiAdjustment:Percent = 0%)         // v9: used for expert tuning (change the RoI cone) 
...
Economy <: thing(
   ...
   decayTable:Affine,           // decay of GDP (function of time), depends on maturity
   investG:Price,               // T$
   ...
   sobriety:list<Energy>,         // part of cancels that is voluntary
   inputs:list<Energy>,                 // higher number than consos because of substitution ??
   maxout:list<Price>,                  // what we would produce without cancellation/warming impact
   results:list<Price>,                 // actual economy gdp with energy cancellation & disaster losses
...
WorldClass <: thing(
   ...
   energy4steel:Affine,           // energy needed to produce 1 ton of steel  (in GJ/ton, we use only a ratio)
   steelPrice:Price,              // price of steel (in $/ton)
   inflation:StepFunction,              // inflation rate when we want a GDP in current dollars
   returnOnInvestment:Percent,     // return on investment (ideal value)
   adaptGrowthLoss:Percent,        // faction of adaptation that reduces the growth invest
   competitivenessFactor:Affine,   // competitive booster due to low labor cost
   ...
   agroEfficiency:Affine,         // efficiency of agriculture (as a function of energy price) KNU
   bioHealth:Affine,        // health of biosystem (as a function of CO2)   
   cropYield:Affine,        // productivity through science & best practice (as a function of time) KMP
...
Block <: Economy(
  ...
  roiEfficiency:Affine,                // use of tech and innovation culture to produce RoI (residual tuning)
  needs:list<list<Energy>>,           // depends on the economy (N - 1)
  carbonTaxAmounts:list<Price>,             // amount of tax (T$)
  socialExpenseRatio:Affine,           // part of GDP that is social expense (health, education, welfare)
  socialExpenseValues:list<Percent>,   // v9: dynamic tuning of SocExp based on active population
  ...
  giniStart:Percent,                   // initial Gini index for the block at origin (2010)
  giniLevels:list<Percent>)            // Gini index for the block at year y (adapted from socExp & AI)
...
Earth <: thing(
    ...
    painDelay:integer = 0,             // CCEM v8: it takes years from signal to decision to implementation
    painReplacement:Affine,            // v8 level of pain (%) as a function of AI replacement
    // simulation
...
ChartsSupplier <: Charts(
  ...
  rawNeeds:Tmeasure,              // "net" needs: raw needs before substitution
...
ChartsConsumer <: Charts(
  ...
  carbonTaxAmounts:Tmeasure,             // carbon tax
...
Problem <: thing(
  ...
  seniorExcessMortality:Percent = 98.5%,        // factor that we apply to the diff equation for seniors
  transferPriceSensitivity:Percent = 0%,      // 0% no price sensitivity
  activeToSocial:Percent = 25%,                // 0% no effect of active population change on social expense
  socialToGini:Percent = 0%,                  // 0% no effect of social expense on Gini index
  aiToGini:Percent = 0%,                      // 0% no effect of AI on Gini index
  aiTransitionDuration:integer = 30,  // number of years for AI to replace human labor
  totalInvest:Price,
  totalGrowth:Price,
  totalEInvest:Price)
...
[gdp$(e:Economy,y:Year) : Price 
    -> e.results[y] * CAGR(pb.world.inflation,1,y)]
...
[CAGR(a:StepFunction,origin:Year,y:Year) : Percent
   -> let v := 1.0 in
        (for i in ((origin + 1) .. y) v :* (1 + get(a,yearF(i))),
         v) ]
...
[CAGR(x1:float,x2:float,n:integer) : Percent
