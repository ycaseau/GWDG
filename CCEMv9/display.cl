...
// this file contains the methods that display the results
// it is not used when CCEM is coupled with G2WS - Hence it is not "diet"
// since it will not be compiled to JavaScript
...
// starts a table - n is the size of each column, l is the list of column titles
...
[eTable() 
  -> startTable(10,add(list<string>{string!(s.name) | s in Supplier},"total")),
     for c in Consumer
        (lineTable(string!(c.name) /+ "-" /+ string!(ORIGIN),10,
     ...
     lineTable("total-" /+ string!(ORIGIN),10,
...
[zoom()
 ->  let PMAX2 := 100.0 in
       (pb.priceRange := list<float>{(PMIN + ((PMAX2 * sqr(i)) / sqr(NIS + 1)))
                                      | i in (2 .. (NIS + 1))},
        PMAX2) ]
...
[hist(c:Consumer) : void
  -> printf("------------------ need ~S (GTep) [~I] ---\n",c,princ(pb.comment,40)),
     display(timeSample(list<float>{sumNeeds(c,i) | i in (1 .. pb.year)})),
     printf("------------------ cancels ~S (TWh) -------------------------------------------\n",c),
     display(timeSample(list<float>{c.cancels[i] | i in (1 .. pb.year)})),
     printf("------------------ consommation ~S -------------------------------------------\n",c),
     display(timeSample(list<float>{sumConsos(c,i) | i in (1 .. pb.year)})),
     printf("------------------ gdp ~S (T$) -------------------------------------------\n",c),
     ...
     printf("------------------ CO2 tax ~S (T$) -------------------------------------------\n",c),
     display(timeSample(list<float>{c.carbonTaxAmounts[i]| i in (1 .. pb.year)}))]
...
[tTable() 
  -> printf("Transition for ~A - warning: all figures are in PWh\n", year!(pb.year)),
...
[tTable%() 
  -> startTable(10,list<string>{label(t) | t in pb.transitions}),
     for c in Consumer
        (lineTable(string!(c.name),10,
             list<float>{(100.0 * c.transferRates[pb.year][t.index]) | t in pb.transitions}),
         separation(10,size(pb.transitions)))]
        ...
        (startTable(8,list<string>{string!(year!(i)) | i in lyears}),
         for c in Consumer
          (lineTable(string!(c.name),8,
           ...
           lineTable(string!(c.name) /+ "%",8,
                    list<float>{(100.0 * c.ePWhs[i] / sumConsos(c,i)) | i in lyears}),
           // debug code (look at the corrections for transfers)
           lineTable(string!(c.name) /+ "trans",8,
...
// gTable() shows the gdp growth
[grTable()
    -> let lyears := list{(1 + (i - 1) * (pb.year - 1) / 9) | i in (2 .. 10)} in
        (startTable(8,list<string>{string!(year!(i)) | i in lyears}),
         for b in Block
          (lineTable(string!(b.describes.name) /+ "-grow",8,
                     list<float>{growth%(b,i) | i in lyears}),
           lineTable(string!(b.describes.name) /+ "-inv%",8,
                     list<float>{invest%(b,i) |  i in lyears}),
          lineTable(string!(b.describes.name) /+ "-res%",8,
                     list<float>{roi%(b,i) |  i in lyears}),
          lineTable(string!(b.describes.name) /+ "-roI%",8,
                     list<float>{(100.0 * roI(b,i)) |  i in lyears}),
           separation(8,length(lyears))))]

// three auxiliary functions : growth%, invest%, roi%
[growth%(b:Block,y:Year) : float
  -> if (y = 1) 0.0 
     else 100.0 * (b.results[y] - b.results[y - 1]) / b.results[y - 1]]
[invest%(b:Block,y:Year) : float
  -> if (y = 1) 0.0 
     else 100.0 * b.investGrowth[y] / b.results[y - 1]]
[roi%(b:Block,y:Year) : float
  -> if (y = 1) 0.0 
     else 100.0 * (b.results[y] - b.results[y - 1]) / b.investGrowth[y]]
...
// CCEM v9 : Population stats
// zoom for one zone, shows the conso for each supplier
[pTable()
  -> let lyears := list(1,21,41,71,91) in
        (startTable(8,list<string>{string!(year!(i)) | i in lyears}),
         for c in Consumer
          (lineTable(string!(c.name),8,
                     list<float>{ activePop%(c,i) | i in lyears}),
           separation(8,length(lyears))),
         for c in Consumer
          (lineTable(string!(c.name) /+ "<20",8,
                     list<float>{ c.youngs[i] | i in lyears}),
           separation(8,length(lyears))),
            for c in Consumer
          (lineTable(string!(c.name) /+ ">60",8,
                     list<float>{ c.seniors[i] | i in lyears}),
           separation(8,length(lyears))))]
        
[iTable()
  -> let lyears := list(1,21,41,71,91) in
        (startTable(10,list<string>{string!(year!(i)) | i in lyears}),
         for c in Consumer
          (lineTable(string!(c.name) /+ "-SX",10,
                     list<float>{ c.economy.socialExpenseValues[i] | i in lyears}),
           separation(10,length(lyears))),
         for c in Consumer
          (lineTable(string!(c.name) /+ "gini",10,
                     list<float>{ c.economy.giniLevels[i] | i in lyears}),
           separation(10,length(lyears))))]
  ...
  -> let p := fopen("data" / "log" /+ string!(integer!(Version * 10.0)),"a"), y := pb.year in
       (use_as_output(p),
        ...
        printf("   cur GDP = ~F2T$, ~I(T$xPWh)\n",gdp$(pb.year),
               for c in Consumer printf("~S:~F2x~F2, ",c,gdp$(c,pb.year),sumConsos(c,pb.year))),
        fclose(p))
...
// ------------------------- our reusable trick to extent a compiled file with interpreted code ------------------

// we load a file of interpreted code - called scenario, used for sensitivity analysis
