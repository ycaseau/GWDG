...
[topt(c:Consumer) : void
  -> //[0] --- start topt(~S), look at satisfaction ----------------------------- // c,
     tacResults(c),
     time_set(),
     bestResponse(BR,c),
     time_show(),
...
[tacResults(c:Consumer) : void
  -> c.cursat := satScore(c),
     ...
     printf("tax tactic: ~F% of pain -> ~F2T$ yearly average\n", c.tactic.taxFromPain,
              average(list<float>{c.carbonTaxAmounts[i] | i in (1 .. pb.year)})),
     printf("cancel tactic: ~F% of pain -> ~F2PWh \n", c.tactic.cancelFromPain,
              average(list<float>{c.economy.cancels[i] | i in (1 .. pb.year)})),
...
// ***************************************************************~*************
// *    Part 3: Evolutionary Search for Nash Equilibrium (TBC in CCEM v10)      *                                     *
// *****************************************************************************
