...
[strategy(tGdp:Percent,tCO2:float,tHappy:float,wGDP:Percent,wCO2:Percent) : Strategy
  -> Strategy(targetGdp = tGdp, targetCO2 = tCO2, targetPain = 1.0 - tHappy, 
              weightGDP = wGDP, weightCO2 = wCO2, weightPain = 1 - (wGDP + wCO2)) ]

// prints a strategy
[self_print(x:Strategy) : void
  -> printf("strategy(Gdp:~F%x~F%,CO2:~F1x~F%,Pain:~F%x~F%)",x.targetGdp,x.weightGDP,x.targetCO2,x.weightCO2,x.targetPain,(1.0 - x.weightGDP - x.weightCO2)) ]
