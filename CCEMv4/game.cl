...
SHOW2:integer :: 1        // verbosity for model M2
...
[getNeed(c:Consumer,y:Year) : void
  -> let b := c.economy,
      ...
      (//[SHOW2] === needs(~S) = ~F2 GTep vs ~F2 (gdp:~F%,pop:~F%) // c,c2,c0,globalEconomyRatio(b,y),populationRatio(b,y),
       //[TALK] [~A] computes ~S needs: ~F3 -> ~F2 (~F%) demat:~F% // year!(y),c,c0,c2,(c2 / c0),dmr,
...
[globalEconomyRatio(w:Block,y:Year) : Percent
  -> localEconomyRatio(w,y) +
     sum(list<Percent>{ (economyRatio(w2,y) * pb.trade[index(w)][index(w2 as Block)] * 
                                              w2.openTrade[index(w2 as Block)]) |
                        w2 in (Block but w)}) ]
...
[updateRate(c:Consumer,s1:Supplier,tr:Transition,y:Year,cneed:Energy) 
   -> let ftech := (1 - pb.world.techFactor) ^ float!(y),
          ...
          w2 := max(w1, get(c.subMatrix[tr.index],yearF(y))),     // transfer expected for this year (monotonic !)
         ...
         (c.substitutions[y][i] := w1 * cneed,                    // actual substitution (rate of previous year)
          ...
          if (s1 = TESTE | s2 = TESTE) 
             (trace(0,"[~A] ~S transfer ~F2GTep(~F%) [~F% now on] of ~S to ~S [matrix ->~F%]\n",
                    year!(y),c,w1 * cneed,w1,w3,s1,tr.to,get(c.subMatrix[tr.index],yearF(pb.year))),
...
[newMaxout(b:Block,y:Year) : Price
  -> (b.maxout[y - 1] * populationGrowth(b,y) * ((1.0 - b.disasterRatios[y]) / (1.0 - b.disasterRatios[y - 1])))
      + b.investGrowth[y - 1] * get(b.roI,yearF(y)) ]
...
[agroOutput(y:Year) : void
   -> let w := pb.world, e := pb.earth,
        ...
        (w.energySurfaces[y] := w.energySurfaces[y - 1] +  (newClean * get(w.landImpact,yearF(y)) * Gt2km2),
         ...
         trace(TALK,"[~A] ~F2 Gt wheat from ~F% surface ratio and ~F% efficiency \n",
